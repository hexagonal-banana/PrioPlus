...
NS_OBJECT_ENSURE_REGISTERED(RoCEv2ExpressPass);
    ...
    RoCEv2ExpressPass::GetTypeId()
    {
        static TypeId tid = TypeId("ns3::RoCEv2ExpressPass")
                                ...
                                .AddAttribute("MaxAggressiveRatio",
                                              ...
                                              DoubleValue(0.5),
                                              MakeDoubleAccessor(&RoCEv2ExpressPass::m_maxAggressiveRatio),
                                              MakeDoubleChecker<double>(0.0, 1.0))
                                .AddAttribute("MinAggressiveRatio",
                                              "The MinAggressiveRatio, 0.01",
                                              DoubleValue(0.01),
                                              MakeDoubleAccessor(&RoCEv2ExpressPass::m_minAggressiveRatio),
                                              MakeDoubleChecker<double>(0.0, 1.0))
                                .AddAttribute("TargetLossRate",
                                              "The target loss rate",
                                              DoubleValue(0.5),
                                              MakeDoubleAccessor(&RoCEv2ExpressPass::m_targetLossRatio),
                                              MakeDoubleChecker<double>(0.0, 1.0))
                                .AddAttribute("InitAggressiveRatio",
                                              "The initial aggressive ratio",
                                              DoubleValue(0.5),
                                              MakeDoubleAccessor(&RoCEv2ExpressPass::m_aggressiveRatio),
                                              MakeDoubleChecker<double>(0.0, 1.0))
                                .AddAttribute("InitCreditRateRatio",
                                              "The initial credit rate ratio to max credit ratio",
                                              DoubleValue(1),
                                              MakeDoubleAccessor(&RoCEv2ExpressPass::m_initCreditRateRatio),
                                              MakeDoubleChecker<double>(0.0, 1.0))
                                ;
        return tid;
    }

    std::string RoCEv2ExpressPass::GetName() const 
    {
        return "RoCEv2ExpressPass";
    }
    ...
    void RoCEv2ExpressPass::UpdateStateWithRcvACK(Ptr<Packet> ack,
...
void
RoCEv2ExpressPass::SendCreditAck(uint32_t psn)
{
    ...
    std::vector<std::reference_wrapper<const Tag>> packetTags{ctTag,csTag,m_creditPathTag,ipTosTag};
...
void
RoCEv2ExpressPass::RateControl(double lossRatio)
{
    //NS_LOG_FUNCTION(this);
    ...
    //std::cout<<"flow "<<this<<" loss ratio "<<lossRatio<<" , target loss ratio "<<m_targetLossRatio<<std::endl;
    if(lossRatio<=m_targetLossRatio){
        ...
        //std::cout<<"flow "<<this<<" rate increase to "<<newRate<<std::endl;
...
void 
RoCEv2ExpressPass::UpdateStateWithOutbandPkt(Ptr<Packet> packet,
                                     const RoCEv2Header& roce,
                                     const uint32_t senderNextPSN)
{
...
void 
RoCEv2ExpressPass::StartCreditAckLoop(const RoCEv2Header& roce)
{
...
void
RoCEv2ExpressPass::UpdateStateRecvData(Ptr<Packet> packet,
                                         const RoCEv2Header& roce)
{
    NS_LOG_FUNCTION(this << packet);
    CreditSeqTag csTag;
    ...
    NS_ABORT_UNLESS(pktSeq>m_lastRecvCreditSeq);
...
TypeId
RoCEv2ExpressPass::CreditSeqTag::GetTypeId()
{
    static TypeId tid = TypeId("ns3::RoCEv2ExpressPass::CreditSeqTag")
                            .SetParent<Tag>()
                            .SetGroupName("DCB")
...
RoCEv2ExpressPass::CreditSeqTag::CreditSeqTag()
    : m_seq(0)
{
}

RoCEv2ExpressPass::CreditSeqTag::CreditSeqTag(uint64_t seq)
    : m_seq(seq)
{
}

TypeId
RoCEv2ExpressPass::CreditSeqTag::GetInstanceTypeId() const
{
    return GetTypeId();
}

uint64_t
RoCEv2ExpressPass::CreditSeqTag::GetSeq() const
{
    return m_seq;
}

void
RoCEv2ExpressPass::CreditSeqTag::SetSeq(uint64_t seq)
{
    m_seq = seq;
}

uint32_t
RoCEv2ExpressPass::CreditSeqTag::GetSerializedSize() const
{
    return sizeof(m_seq);
}

void
RoCEv2ExpressPass::CreditSeqTag::Serialize(TagBuffer i) const
{
    i.WriteU64(m_seq);
}

void
RoCEv2ExpressPass::CreditSeqTag::Deserialize(TagBuffer i)
{
    m_seq = i.ReadU64();
}

void
RoCEv2ExpressPass::CreditSeqTag::Print(std::ostream& os) const
{
    os << "CreditSeq=" << m_seq;
}
}